# saintcon23/advanced_terraform/intro_to_modules/modules/security_groups/variables.tf

#####################
## Security Groups ##
#####################
variable "vpc_id" {
  description = "The vpc id"
  type        = string
  default     = ""
}
variable "sg_cidr_blocks_allow_ssh" {
  description = "The CIDR blocks of the security group"
  type        = list(string)
  # default     = [""]
}
variable "sg_cidr_blocks_allow_http" {
  description = "The CIDR blocks of the security group"
  type        = list(string)
  # default     = [""]
}
variable "sg_cidr_blocks_allow_https" {
  description = "The CIDR blocks of the security group"
  type        = list(string)
  # default     = [""]
}
variable "sg_cidr_blocks_allow_rdp" {
  description = "The CIDR blocks of the security group"
  type        = list(string)
  # default     = [""]
}
variable "sg_ipv6_cidr_blocks_allow_ssh" {
  description = "The ipv6 CIDR blocks of the security group"
  type        = list(string)
  # default     = [""]

}
variable "sg_ipv6_cidr_blocks_allow_http" {
  description = "The ipv6 CIDR blocks of the security group"
  type        = list(string)
  # default     = [""]
}
variable "sg_ipv6_cidr_blocks_allow_https" {
  description = "The ipv6 CIDR blocks of the security group"
  type        = list(string)
  # default     = [""]
}
variable "sg_ipv6_cidr_blocks_allow_rdp" {
  description = "The ipv6 CIDR blocks of the security group"
  type        = list(string)
  # default     = [""]
}
##########
## TAGS ##
##########
variable "security_tags" {
  description = "security group tags"
  type        = map(string)
  default     = {}
}